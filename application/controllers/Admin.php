	...
	public function unitkerjakabkotadetail()
	{
		...
		if ($mau_ke == "pilih_kegiatan")
		{
			if($id_bulan == '00')
			{
				...
				$a['data3']		= $this->db->query("SELECT * FROM (SELECT t.id_jeniskegiatan AS idkgt, t.target AS target_mingguan, t.realisasi AS real_mingguan, t.batas_minggu, m.mulai,m.batas_waktu, u.tim, subk.nama_kegiatan AS nmkegiatan,k.id_tim, t.ketua_tim AS ketua FROM m_jeniskegiatan m LEFT JOIN m_listkegiatan subk ON m.nama_kegiatan = subk.id LEFT JOIN m_kegiatan k ON subk.id_kegiatan = k.id_kegiatan LEFT JOIN m_tim u ON k.id_tim = u.id_tim LEFT JOIN m_satuan s ON m.satuan = s.id_satuan LEFT JOIN t_kegiatan t ON m.id_jeniskegiatan = t.id_jeniskegiatan WHERE (MONTH(m.mulai) <= '$bulanini' AND MONTH(m.batas_waktu) >= '$bulanini') AND MONTH(t.batas_minggu) = '$id_bulan' AND YEAR(m.batas_waktu) = '$tahun' AND t.id_kab = $id_kabkota) AS kegiatan JOIN (SELECT id_jeniskegiatan, SUM(target) AS total_target, SUM(realisasi) AS total_realisasi FROM t_kegiatan WHERE id_kab = '$id_kabkota' GROUP BY id_jeniskegiatan) AS total_target ON kegiatan.idkgt = total_target.id_jeniskegiatan ORDER BY kegiatan.id_tim ASC, kegiatan.idkgt, kegiatan.batas_minggu ASC;;")->result();
		...
		else
		{
			...
			$a['data3']		= $this->db->query("SELECT * FROM (SELECT t.id_jeniskegiatan AS idkgt, t.target AS target_mingguan, t.realisasi AS real_mingguan, t.batas_minggu, m.mulai,m.batas_waktu, u.tim, subk.nama_kegiatan AS nmkegiatan,k.id_tim, t.ketua_tim AS ketua FROM m_jeniskegiatan m LEFT JOIN m_listkegiatan subk ON m.nama_kegiatan = subk.id LEFT JOIN m_kegiatan k ON subk.id_kegiatan = k.id_kegiatan LEFT JOIN m_tim u ON k.id_tim = u.id_tim LEFT JOIN m_satuan s ON m.satuan = s.id_satuan LEFT JOIN t_kegiatan t ON m.id_jeniskegiatan = t.id_jeniskegiatan WHERE (MONTH(m.mulai) <= '$bulanini' AND MONTH(m.batas_waktu) >= '$bulanini') AND MONTH(t.batas_minggu) = '$bulanini' AND YEAR(m.batas_waktu) = '$tahun' AND t.id_kab = $id_kabkota) AS kegiatan JOIN (SELECT id_jeniskegiatan, SUM(target) AS total_target, SUM(realisasi) AS total_realisasi FROM t_kegiatan WHERE id_kab = '$id_kabkota' GROUP BY id_jeniskegiatan) AS total_target ON kegiatan.idkgt = total_target.id_jeniskegiatan ORDER BY kegiatan.id_tim ASC, kegiatan.idkgt, kegiatan.batas_minggu ASC;;")->result();
			$a['page']			= "l_unitkerjakabkotadetail";	
		}
